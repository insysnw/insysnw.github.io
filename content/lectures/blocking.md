...
С наибольшей вероятностью у вас эта программа, как и у нас, выведет **Yay**, что значит, что сокет успешно создан.

Рассмотрим подробнее функцию `socket`.
У неё есть три аргумента:

* domain - семейство абстрагиремых каналов связи (это может быть [IR], bluetooth, так и IPv4 стэк)
...
И каждому типу сокета соответствует только один протоков TCP для потоковых, UDP для датаграммных.
...
Ещё одни важный аспект этой функции - её возвращаемое значение, файловый дескритор.
Вот оно, сходство с файлом!
Дальше можно передавать файловый дескритор во все функции, что умеют с ним работать.
